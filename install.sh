        ...
        try {
            ...
            for(let key of pendingKeys) {
                ...
                if (Date.now() > req.expired_at) {
                    ...
                    // Menggunakan Logika String Terbukti Sukses + Pengaman Blacklist SQLite
                    if (gopayRes && gopayRes.data) {
                        // Ubah seluruh respons BHM Biz jadi teks mentah
                        let strData = JSON.stringify(gopayRes.data);
                        
                        // Siapkan format pencarian sesuai dokumentasi BHM Biz (ada .00)
                        let search1 = `"${req.amount_to_pay}"`;      // "1000"
                        let search2 = `:${req.amount_to_pay}`;       // :1000
                        let search3 = `"${req.amount_to_pay}.00"`;   // "1000.00"
                        let search4 = `:${req.amount_to_pay}.00`;    // :1000.00

                        // Jika nominal pelanggan ketemu di dalam riwayat GoPay
                        if (strData.includes(search1) || strData.includes(search2) || strData.includes(search3) || strData.includes(search4)) {
                            
                            // 🛡️ KUNCI PENGAMAN (BLACKLIST): Jadikan nominal sebagai ID Unik
                            let mutIdFallback = "LOCKED_NOMINAL_" + req.amount_to_pay;
                            
                            // Cek ke database SQLite: Apakah nominal unik ini sudah pernah masuk saldo?
                            let isUsed = dbSqlite.prepare("SELECT id FROM used_mutations WHERE id = ?").get(mutIdFallback);
                            
                            // Jika belum pernah dipakai, berikan saldonya!
                            if (!isUsed) {
                                isFound = true;
