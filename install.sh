...
EOF
    ...
    # SERVICE WORKER BARU ANTI-MACET (BYPASS CACHE)
...
self.addEventListener('fetch', (e) => {
    // Memaksa browser menarik data baru (menghentikan loop muter-muter)
...
<head>
    ...
    <style>
        ...
        /* PERBAIKAN TOMBOL PROFIL YANG TIDAK BISA DIKLIK */
        .prof-actions-container {
            padding: 0 20px;
            margin-bottom: 150px; /* Ruang ekstra untuk mengindari Floating Bottom Nav */
            display: flex;
            flex-direction: column;
            gap: 10px;
            position: relative;
            z-index: 50; /* Memaksa tombol di atas layer navigasi */
        }
...
for (let userPhone in dbInitial) {
    ...
    if (pwd && pwd.length !== 64) { // Jika panjang bukan 64, berarti belum di hash SHA-256
...
// Blokir Akses Direct ke File JSON dari Browser Hacker
app.get('*.json', (req, res, next) => {
    if (req.path.includes('manifest.json')) return next();
...
// Routing tangkapan jatuh bebas agar Reload tidak nyangkut!
app.get('*', (req, res) => {
