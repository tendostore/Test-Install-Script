...
<body class="dark-mode"> <div id="app">
    ...
    <script>
        ...
        // MANAJEMEN MODAL & TOMBOL BACK HP (POPSTATE EVENT)
        let modalStack = [];
        ...
        function openTopupModal() { 
            ...
            modalStack.push('topup-modal'); history.pushState({ modalId: 'topup-modal' }, "", "#topup-modal");
        }
        ...
        async function generateQris() {
            ...
            try {
                ...
                if(data && data.success) { 
                    document.getElementById('topup-modal').classList.add('hidden');
                    // Menggantikan modal topup di history dengan success modal
                    modalStack[modalStack.length-1] = 'topup-success-modal'; 
        ...
        function openHistoryDetail(h) {
            ...
            modalStack.push('history-detail-modal'); history.pushState({ modalId: 'history-detail-modal' }, "", "#history-detail-modal");
        }
            ...
            if(confirm("Yakin ingin membatalkan topup ini?")) {
                ...
                try {
                    ...
                    if(res.success) {
                        ...
                        // Close history detail programmatically
                        document.getElementById('history-detail-modal').classList.add('hidden'); 
                        history.back();
                        syncUserData();
                    } else {
        ...
        window.openEditModal = function(type) {
            ...
            modalStack.push('edit-modal'); history.pushState({ modalId: 'edit-modal' }, "", "#edit-modal");
        };
        ...
        function openOrderModal(sku, nama, harga, desc) {
            ...
            modalStack.push('order-modal'); history.pushState({ modalId: 'order-modal' }, "", "#order-modal");
        }
            ...
            try {
                ...
                if(data && data.success) {
                    ...
                    if (method === 'qris') {
                        ...
                        modalStack[modalStack.length-1] = 'order-success-modal';
        ...
        function openVPNServerSelection(protocol) {
            ...
            modalStack.push('vpn-server-modal'); history.pushState({ modalId: 'vpn-server-modal' }, "", "#vpn-server-modal");
        }
        ...
        function openVPNTrialModal(productId, protocol, customName) {
            ...
            if (modalStack[modalStack.length - 1] === 'vpn-server-modal') {
                modalStack[modalStack.length - 1] = 'vpn-trial-modal';
            } else {
                modalStack.push('vpn-trial-modal');
                history.pushState({ modalId: 'vpn-trial-modal' }, "", "#vpn-trial-modal");
            }
        ...
        function openVPNOrderModal(productId, protocol, price, desc, customName) {
            ...
            if (modalStack[modalStack.length - 1] === 'vpn-server-modal') {
                modalStack[modalStack.length - 1] = 'vpn-order-modal';
            } else {
                modalStack.push('vpn-order-modal');
                history.pushState({ modalId: 'vpn-order-modal' }, "", "#vpn-order-modal");
            }
...
function sendTelegramAdmin(message) {
    try {
        ...
        } else if (cfg.teleToken && cfg.teleChatId) {
...
function sendBroadcastSuccess(productName, rawUser, rawTarget, price, method) {
    try {
        ...
        // Broadcast to Telegram Channel/Group
        ...
        } else if (cfg.teleTokenInfo && cfg.teleChannelId) {
             // Fallback
        ...
        // Broadcast to WA Group
        let targetWaGroup = cfg.waBroadcastGroup || cfg.waBroadcastId;
        if (globalSock && targetWaGroup) {
            let msgWa = `✅ *PEMBELIAN BERHASIL*\n\n👤 Pelanggan: ${rawUser}\n📦 Layanan: ${productName}\n🎯 Tujuan: ${maskTarget}${priceStr}${methodStr}\n🕒 Waktu: ${timeStr} WIB\n\n_🌐 Transaksi diproses otomatis oleh sistem._`;
            globalSock.sendMessage(targetWaGroup, { text: msgWa }).catch(e => {});
        }
    } catch(e) {}
}
...
// Modifikasi Endpoint Produk untuk unshift is_custom_top == 1
app.get('/api/produk', (req, res) => { 
    try {
        let semuaProduk = getAllRecords('produk'); 
        let sortedProduk = {};
        let topCustom = [];
        let normal = [];
        
        for (let sku in semuaProduk) {
            if (semuaProduk[sku].is_custom_top === 1) {
                topCustom.push({ key: sku, data: semuaProduk[sku] });
            } else {
                normal.push({ key: sku, data: semuaProduk[sku] });
            }
        }
        
        topCustom.forEach(item => { sortedProduk[item.key] = item.data; });
        normal.forEach(item => { sortedProduk[item.key] = item.data; });
        
        res.json(sortedProduk);
    } catch(e) {
        res.json({});
    }
});
...
async function tarikDataLayananOtomatis() {
    try {
        ...
        Object.keys(produkLama).forEach(k => {
            // Keep custom products (is_manual_cat or is_custom_top)
        ...
        daftarPusat.forEach(item => {
            ...
            for (let k in daftarLokal) {
                if (String(daftarLokal[k].sku_asli).toUpperCase() === String(kodeBarang).toUpperCase()) {
                    daftarLokal[k].harga = finalPrice;
                    if(daftarLokal[k].is_custom_top) daftarLokal[k].nama = namaBarang; 
                }
            }
...
console.log('\x1b[36m🔑 Sesi OTP Aktif (In-DB)   : \x1b[1m' + optCount + '\x1b[0m');
        ...
        } catch(e) { console.log('\x1b[31mGagal memuat statistik:\x1b[0m', e.message); }
    "
    echo ""
    read -p "Tekan Enter untuk kembali..."
}
...
menu_manajemen_produk_instan() {
    while true; do
        ...
        echo -e "${C_CYAN}${C_BOLD}======================================================${C_RST}"
        echo -e "  ${C_GREEN}[1]${C_RST} Tambah Produk Instan (Ambil Harga Digiflazz)"
        echo -e "  ${C_GREEN}[2]${C_RST} Hapus Produk Instan"
        echo -e "  ${C_GREEN}[3]${C_RST} Edit Nama/Kategori Produk"
        echo -e "  ${C_GREEN}[4]${C_RST} Daftar Produk Instan (Custom Top)"
        echo -e "${C_CYAN}------------------------------------------------------${C_RST}"
        ...
        case $prod_choice in
            1)
                echo -e "\n${C_MAG}--- TAMBAH PRODUK INSTAN ---${C_RST}"
                read -p "Masukkan KODE SKU Digiflazz: " sku_input
                if [ -z "$sku_input" ]; then echo "Batal."; sleep 1; continue; fi
                read -p "Masukkan Nama Produk (Tampil di Web): " nama_input
                read -p "Masukkan Kategori (Misal: Pulsa, Data, E-Money): " kategori_input

                SKU="$sku_input" NAMA="$nama_input" KATEGORI="$kategori_input" node -e "
                    const Database = require('better-sqlite3');
                    const axios = require('axios');
                    const crypto = require('crypto');
                    ...
                    async function tambah() {
                        ...
                        let user = config.digiflazzUsername || '';
                        let key = config.digiflazzApiKey || '';
                        if(!user || !key) { console.log('\x1b[31mAPI Digiflazz belum diatur di config!\x1b[0m'); return; }

                        let skuInput = process.env.SKU;
                        let sign = crypto.createHash('md5').update(user + key + 'pricelist').digest('hex');
                        
                        try {
                            console.log('\x1b[36m⏳ Mencari SKU di Digiflazz...\x1b[0m');
                            let res = await axios.post('https://api.digiflazz.com/v1/price-list', { cmd: 'prepaid', username: user, sign: sign });
                            let data = res.data.data || [];
                            let target = data.find(p => String(p.buyer_sku_code).toUpperCase() === String(skuInput).toUpperCase());
                            ...
                            if(!target) {
                                ...
                                target = dataPasca.find(p => String(p.buyer_sku_code).toUpperCase() === String(skuInput).toUpperCase());
                            }

                            if(!target) {
                                console.log('\x1b[31m❌ SKU ' + skuInput + ' tidak ditemukan di Digiflazz.\x1b[0m');
                                return;
                            }

                            let hargaModal = target.price || target.admin || 0;
                            let m = config.margin || { t1:50, t2:100, t3:250, t4:500, t5:1000, t6:1500, t7:2000, t8:2500, t9:3000, t10:4000, t11:5000, t12:7500, t13:10000 };
                            let keuntungan = 0;
                            if(hargaModal <= 100) keuntungan = m.t1;
                            else if(hargaModal <= 500) keuntungan = m.t2;
                            ...
                            let finalPrice = hargaModal + keuntungan;
                            let obj = {
                                sku_asli: target.buyer_sku_code,
                                nama: process.env.NAMA !== '' ? process.env.NAMA : target.product_name,
                                harga: finalPrice,
                                kategori: process.env.KATEGORI !== '' ? process.env.KATEGORI : 'Instan',
                                brand: target.brand || 'Instan',
                                sub_kategori: target.type || 'Umum',
                                deskripsi: target.desc || 'Produk Instan Prioritas',
                                status_produk: (target.buyer_product_status && target.seller_product_status),
                                is_manual_cat: false,
                                is_custom_top: 1
                            };

                            db.prepare('INSERT OR REPLACE INTO produk (id, data) VALUES (?, ?)').run(obj.sku_asli, JSON.stringify(obj));
                            console.log('\x1b[32m✅ Produk ' + obj.nama + ' (SKU: ' + obj.sku_asli + ') berhasil ditambahkan!\x1b[0m');
                            console.log('💰 Harga Modal: Rp ' + hargaModal.toLocaleString('id-ID') + ' | Jual: Rp ' + finalPrice.toLocaleString('id-ID'));

                        } catch(e) {
                            console.log('\x1b[31mError memanggil API: ' + e.message + '\x1b[0m');
                        }
                    }
                    tambah();
                "
            ...
            2)
                ...
                read -p "Masukkan KODE SKU yang ingin dihapus: " sku_hapus
                SKU="$sku_hapus" node -e "
                    const Database = require('better-sqlite3');
                    const db = new Database('tendo_database.db');
                    let sku = process.env.SKU;
                    let row = db.prepare('SELECT data FROM produk WHERE id = ?').get(sku);
                    if(row) {
                        let p = JSON.parse(row.data);
                        if(p.is_custom_top === 1) {
                            db.prepare('DELETE FROM produk WHERE id = ?').run(sku);
                            console.log('\x1b[32m✅ Produk ' + sku + ' berhasil dihapus dari daftar prioritas!\x1b[0m');
                        } else {
                            console.log('\x1b[33mProduk ditemukan tapi bukan produk instan prioritas. Gunakan Sinkronisasi untuk mengatur ulang.\x1b[0m');
                        }
                    } else {
            ...
            3)
                ...
                read -p "Masukkan KODE SKU: " sku_edit
                read -p "Nama Baru (Kosongkan jika tidak ubah): " nama_edit
                read -p "Kategori Baru (Kosongkan jika tidak ubah): " kat_edit
                SKU="$sku_edit" NAMA="$nama_edit" KAT="$kat_edit" node -e "
                    const Database = require('better-sqlite3');
                    const db = new Database('tendo_database.db');
                    let sku = process.env.SKU;
                    let row = db.prepare('SELECT data FROM produk WHERE id = ?').get(sku);
                    if(row) {
                        ...
                        if(p.is_custom_top === 1) {
                            ...
                            db.prepare('UPDATE produk SET data = ? WHERE id = ?').run(JSON.stringify(p), sku);
                            console.log('\x1b[32m✅ Produk ' + sku + ' berhasil diupdate!\x1b[0m');
                        } else {
                            console.log('\x1b[33mBukan produk instan custom.\x1b[0m');
                        }
                    } else {
                        console.log('\x1b[31mSKU tidak ditemukan.\x1b[0m');
                    }
            ...
            4)
                ...
                node -e "
                    ...
                    for(let r of rows) {
                        ...
                        if(p.is_custom_top === 1) {
                            count++;
                            console.log('- [\x1b[33m' + p.sku_asli + '\x1b[0m] \x1b[36m' + p.nama + '\x1b[0m (Kategori: ' + p.kategori + ') | Rp ' + p.harga.toLocaleString('id-ID'));
                        }
                    }
                    if(count === 0) console.log('\x1b[33mBelum ada produk instan custom.\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            0) break ;;
...
menu_notifikasi() {
    while true; do
        ...
        echo -e "${C_CYAN}${C_BOLD}======================================================${C_RST}"
        echo -e "  ${C_GREEN}[1]${C_RST} Setup Telegram Admin (Laporan)"
        echo -e "  ${C_GREEN}[2]${C_RST} Setup Telegram Channel/Grup Pelanggan"
        echo -e "  ${C_GREEN}[3]${C_RST} Setup WhatsApp Broadcast Group"
        echo -e "  ${C_GREEN}[4]${C_RST} Hapus Semua Pemberitahuan Web"
        echo -e "${C_CYAN}------------------------------------------------------${C_RST}"
        echo -e "  ${C_RED}[0]${C_RST} Kembali"
        echo -e "${C_CYAN}======================================================${C_RST}"
        echo -ne "${C_YELLOW}Pilih menu [0-4]: ${C_RST}"
        ...
        case $notif_choice in
            1)
                read -p "Masukkan Telegram Bot Token Admin: " t_token
                read -p "Masukkan Chat ID Admin: " t_id
                T_TOKEN="$t_token" T_ID="$t_id" node -e "
                    const Database = require('better-sqlite3');
                    const db = new Database('tendo_database.db');
                    let row = db.prepare(\"SELECT data FROM config WHERE id = 'main'\").get();
                    let config = row ? JSON.parse(row.data) : {};
                    config.teleTokenAdmin = process.env.T_TOKEN;
                    config.teleChatIdAdmin = process.env.T_ID;
                    db.prepare(\"INSERT OR REPLACE INTO config (id, data) VALUES ('main', ?)\").run(JSON.stringify(config));
                    console.log('\x1b[32m✅ Konfigurasi Telegram Admin disimpan!\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            2)
                read -p "Masukkan Telegram Link/Username/ID Pelanggan (Grup/Channel): " t_link
                T_LINK="$t_link" node -e "
                    const Database = require('better-sqlite3');
                    const db = new Database('tendo_database.db');
                    let row = db.prepare(\"SELECT data FROM config WHERE id = 'main'\").get();
                    let config = row ? JSON.parse(row.data) : {};
                    config.teleLinkPelanggan = process.env.T_LINK;
                    db.prepare(\"INSERT OR REPLACE INTO config (id, data) VALUES ('main', ?)\").run(JSON.stringify(config));
                    console.log('\x1b[32m✅ Konfigurasi Telegram Pelanggan disimpan!\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            3)
                read -p "Masukkan ID Grup WhatsApp (contoh: 120363xxx@g.us): " wa_grp
                WA_GRP="$wa_grp" node -e "
                    const Database = require('better-sqlite3');
                    const db = new Database('tendo_database.db');
                    let row = db.prepare(\"SELECT data FROM config WHERE id = 'main'\").get();
                    let config = row ? JSON.parse(row.data) : {};
                    config.waBroadcastGroup = process.env.WA_GRP;
                    // Sinkronkan juga field lama waBroadcastId untuk kompabilitas
                    config.waBroadcastId = process.env.WA_GRP;
                    db.prepare(\"INSERT OR REPLACE INTO config (id, data) VALUES ('main', ?)\").run(JSON.stringify(config));
                    console.log('\x1b[32m✅ Konfigurasi Grup WA Broadcast disimpan!\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            4)
                node -e "
                    ...
                    db.prepare(\"DELETE FROM sqlite_sequence WHERE name='web_notif'\").run();
                    console.log('\x1b[32m✅ Semua notifikasi (Pemberitahuan) di Web berhasil dihapus!\x1b[0m');
                "
                read -p "Tekan Enter untuk kembali..."
                ;;
            0) break ;;
...
menu() {
    while true; do
        ...
        echo -e "${C_CYAN}${C_BOLD}======================================================${C_RST}"
        echo -e "  ${C_GREEN}[1]${C_RST}  Manajemen Member & Saldo"
        echo -e "  ${C_GREEN}[2]${C_RST}  Manajemen Etalase Custom"
        echo -e "  ${C_GREEN}[3]${C_RST}  Manajemen Produk Instan (Custom Top)"
        echo -e "  ${C_GREEN}[4]${C_RST}  Manajemen VPN Premium"
        echo -e "  ${C_GREEN}[5]${C_RST}  Manajemen Tutorial Web"
        echo -e "  ${C_GREEN}[6]${C_RST}  Setup Notifikasi & Broadcast"
        echo -e "  ${C_GREEN}[7]${C_RST}  Setup Keuntungan (Margin PPOB)"
        echo -e "  ${C_GREEN}[8]${C_RST}  Sinkronisasi Produk Digiflazz"
        echo -e "  ${C_GREEN}[9]${C_RST}  Konfigurasi API Gateway (Digi/GoPay)"
        echo -e "  ${C_GREEN}[10]${C_RST} Setup Waktu Pemeliharaan Web"
        echo -e "  ${C_GREEN}[11]${C_RST} Setup Nomor CS/Admin"
        echo -e "  ${C_GREEN}[12]${C_RST} Auto-Backup Telegram"
        echo -e "  ${C_GREEN}[13]${C_RST} Backup & Restore Data Manual"
        echo -e "  ${C_GREEN}[14]${C_RST} Statistik Database SQLite"
        echo -e "  ${C_GREEN}[15]${C_RST} Monitor Log Sistem Realtime"
        echo -e "  ${C_GREEN}[16]${C_RST} Restart Semua Layanan (Bot & Nginx)"
        echo -e "${C_CYAN}------------------------------------------------------${C_RST}"
        ...
        case $choice in
            1) menu_member ;;
            2) menu_etalase_custom ;;
            3) menu_manajemen_produk_instan ;;
            4) menu_manajemen_vpn ;;
            5) menu_tutorial ;;
            6) menu_notifikasi ;;
            7) menu_keuntungan ;;
            8) menu_sinkron ;;
            9) menu_config_api ;;
            10) menu_pemeliharaan ;;
            11) menu_setup_cs ;;
            12) menu_telegram ;;
            13) menu_backup ;;
            14) menu_db_stats ;;
            15) menu_cek_log ;;
            16) menu_restart_services ;;
            0) clear; echo -e "${C_GREEN}Keluar dari panel admin...${C_RST}"; exit 0 ;;
            *) echo -e "${C_RED}❌ Pilihan tidak valid!${C_RST}"; sleep 1 ;;
        esac
    done
}
