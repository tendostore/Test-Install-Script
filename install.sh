...
if [[ "$dom_opt" == "1" ]]; then
    ...
    USE_CF="false"
    echo "Pastikan domain $DOMAIN_INIT sudah di-pointing ke IP VPS ini!"
    sleep 3
else
    DOMAIN_INIT="vpn-$(tr -dc a-z0-9 </dev/urandom | head -c 5).vip3-tendo.my.id"
    USE_CF="true"
    echo "Auto Domain akan dibuat: $DOMAIN_INIT"
    sleep 2
fi
...
gnupg1 bc iproute2 iptables iptables-persistent python3 neofetch zip unzip >/dev/null 2>&1
...
# Daemon untuk mencatat IP login Xray
...
EOF
systemctl daemon-reload
systemctl enable xray-login-notif
systemctl restart xray-login-notif

# Script Notifikasi Login (berjalan sesuai cron)
...
# Script Auto Backup (berjalan sesuai cron)
...
function header_main() {
    ...
    CITY=$(cat /root/tendo/city 2>/dev/null)
    ISP=$(cat /root/tendo/isp 2>/dev/null)
    IP=$(cat /root/tendo/ip 2>/dev/null)
    
    IFACE=$(ip -4 route ls | grep default | grep -Po '(?<=dev )(\S+)' | head -1)
    MON_DATA=$(vnstat -m -i $IFACE --oneline | awk -F';' '{print $11}')
    M_RX=$(vnstat -m -i $IFACE --oneline | awk -F';' '{print $9}')
    M_TX=$(vnstat -m -i $IFACE --oneline | awk -F';' '{print $10}')
    DAY_DATA=$(vnstat -d -i $IFACE --oneline | awk -F';' '{print $6}')
    D_RX=$(vnstat -d -i $IFACE --oneline | awk -F';' '{print $4}')
    D_TX=$(vnstat -d -i $IFACE --oneline | awk -F';' '{print $5}')
    ...
    SX=$(systemctl is-active xray)
    [[ $SX == "active" ]] && X_ST="${GREEN}ON${NC}" || X_ST="${RED}OFF${NC}"
    SZ=$(systemctl is-active zivpn)
    [[ $SZ == "active" ]] && Z_ST="${GREEN}ON${NC}" || Z_ST="${RED}OFF${NC}"
    SI=$(systemctl is-active netfilter-persistent)
    [[ $SI == "active" ]] && I_ST="${GREEN}ON${NC}" || I_ST="${RED}OFF${NC}"
    
    COUNT_VLESS=$(jq '.inbounds[0].settings.clients | length' $CONFIG)
...
function backup_restore_menu() {
    while true; do
        ...
        case $opt in
            1)
                clear
                echo -e "${YELLOW}BACKUP DATA VPS${NC}\n"
                rm -f /root/tendo/backup.zip
            ...
            2)
                clear
                echo -e "${YELLOW}RESTORE DATA VPS${NC}\n"
...
function notif_login_menu() {
    while true; do
        clear
        if crontab -l 2>/dev/null | grep -q "/usr/local/bin/login-report.sh"; then 
            ST="ON"
            TG="OFF"
        else 
            ST="OFF"
        ...
        case $opt in
            ...
            2)
                ...
                if [[ "$m" =~ ^[0-9]+$ ]]; then
                    ...
                    echo " Berhasil diatur ke $m menit!"
                    sleep 1
                else
...
function notif_backup_menu() {
    while true; do
        clear
        if crontab -l 2>/dev/null | grep -q "/usr/local/bin/auto-backup.sh"; then 
            ST="ON"
            TG="OFF"
        else 
            ST="OFF"
...
function telegram_bot_menu() {
    while true; do
        ...
        case $opt in
            1) 
                clear
                echo -e "${YELLOW}TUTORIAL MAKE BOT API & CHAT ID${NC}\n"
                ...
                echo -e "5. Kembali ke menu ini, pilih opsi 5 untuk"
            ...
            4) 
                clear
                echo -e "${YELLOW}MANUAL BACKUP VPS TO TELEGRAM${NC}\n"
                TOKEN=$(cat /root/tendo/bot_token 2>/dev/null)
                CHAT_ID=$(cat /root/tendo/chat_id 2>/dev/null)
            ...
            5) 
                clear
                echo -e "${YELLOW}CHANGE BOT API & CHAT ID${NC}\n"
...
function xray_menu() {
    while true; do 
        ...
        echo -e " [5] Check Account Details"
        echo -e " [x] Back"
        echo -e "${CYAN}─────────────────────────────────────────────────${NC}"
        read -p " Select Menu : " opt
        ...
        case $opt in
            1) 
                ...
                echo -e "Expired On     : $ex Hari ($exp_date)"
                echo -e "────────────────────────────────────"
                echo -e "            XRAY WS TLS"
                echo -e "────────────────────────────────────"
                echo -e "$ltls"
                echo -e "────────────────────────────────────"
                echo -e "          XRAY WS NO TLS"
                echo -e "────────────────────────────────────"
                echo -e "$lnon"
                echo -e "────────────────────────────────────"
                read -n 1 -s -r -p "Enter..."
            ...
            2) 
                ...
                echo -e "Network        : ws"
                echo -e "Path ws        : /vless"
                echo -e "Expired On     : $ex_m Menit ($exp_date)"
                echo -e "────────────────────────────────────"
                echo -e "            XRAY WS TLS"
                echo -e "────────────────────────────────────"
                echo -e "$ltls"
            ...
            5) 
                ...
                idx=$((n-1))
                u=$(jq -r ".inbounds[0].settings.clients[$idx].email" $CONFIG)
                id=$(jq -r ".inbounds[0].settings.clients[$idx].id" $CONFIG)
                
                DMN=$(cat /usr/local/etc/xray/domain)
                exp_d=$(grep "^$u|" $U_DATA | cut -d'|' -f3)
                [[ -z "$exp_d" ]] && exp_d="Unknown"
                CTY=$(cat /root/tendo/city)
                ISP=$(cat /root/tendo/isp)
    ...
    while true; do 
        ...
        case $opt in
            1) 
                ...
                systemctl restart zivpn
                echo "$p|$exp" >> /etc/zivpn/user_data.txt
                DMN=$(cat /usr/local/etc/xray/domain)
                ...
                echo -e "━━━━━━━━━━━━━━━━━━━━━"
                echo -e "Password   : $p"
                echo -e "CITY       : $(cat /root/tendo/city)"
                echo -e "ISP        : $(cat /root/tendo/isp)"
                echo -e "IP ISP     : $(cat /root/tendo/ip)"
                echo -e "Domain     : $DMN"
            ...
            2) 
                ...
                echo -e "IP ISP     : $(cat /root/tendo/ip)"
                echo -e "Domain     : $DMN"
                echo -e "Expired On : $ex_m Menit ($exp)"
                echo -e "━━━━━━━━━━━━━━━━━━━━━"
                read -p "Enter..."
                ;;
            3) 
                ...
                [[ -z "$n" ]] && continue
                idx=$((n-1))
                p=$(jq -r ".auth.config[$idx]" /etc/zivpn/config.json)
                sed -i "/^$p|/d" /etc/zivpn/user_data.txt
                jq "del(.auth.config[$idx])" /etc/zivpn/config.json > /tmp/z && mv /tmp/z /etc/zivpn/config.json
                systemctl restart zivpn
                ;;
            4) 
            ...
            5) 
                ...
                p=$(jq -r ".auth.config[$idx]" /etc/zivpn/config.json)
                DMN=$(cat /usr/local/etc/xray/domain)
                exp_d=$(grep "^$p|" /etc/zivpn/user_data.txt | cut -d'|' -f2)
                [[ -z "$exp_d" ]] && exp_d="Unknown"
                
                clear
                echo -e "━━━━━━━━━━━━━━━━━━━━━"
                echo -e "  CHECK ZIVPN UDP"
                echo -e "━━━━━━━━━━━━━━━━━━━━━"
                echo -e "Password   : $p"
                echo -e "CITY       : $(cat /root/tendo/city)"
                echo -e "ISP        : $(cat /root/tendo/isp)"
...
function routing_menu() {
    while true; do 
        header_sub
        echo -e "             SUPPORTED GEOSITE LIST               "
        echo -e "─────────────────────────────────────────────────"
        echo -e " rule-gaming, rule-indo, rule-sosmed, google,    "
        echo -e " rule-playstore, rule-streaming, rule-umum, tiktok,"
        echo -e " rule-ipcheck, rule-doh, rule-malicious, telegram,"
        echo -e " rule-ads, rule-speedtest, ecommerce-id, urltest,"
        echo -e " category-porn, bank-id, meta, videoconference,  "
        echo -e " geolocation-!cn, facebook, spotify, openai, meta,"
        echo -e " ehentai, github, microsoft, apple, netflix, cn, "
        echo -e " youtube, twitter, bilibili, category-ads-all,   "
        echo -e " private, category-media, category-vpnservices,  "
        echo -e " category-dev, category-dev-all, meta, category-media-all"
        echo -e "─────────────────────────────────────────────────"
...
function check_services() {
    ...
    services=("xray" "zivpn" "vnstat" "netfilter-persistent" "xray-login-notif")
    names=("Xray VPN Core   " "ZIVPN UDP Server" "Vnstat Monitor  " "Iptables Rules  " "Login Logger    ")
    
    for i in "${!services[@]}"; do 
        if systemctl is-active --quiet "${services[$i]}"; then 
            status="${GREEN}ACTIVE (ON)${NC}"
        else 
            status="${RED}INACTIVE (OFF)${NC}"
        fi
        echo -e " ${names[$i]} : $status"
    done
...
function features_menu() {
    while true; do
        ...
        case $opt in
            ...
            3) 
                header_sub
                python3 <(curl -sL https://raw.githubusercontent.com/sivel/speedtest-cli/master/speedtest.py) --share
                read -p "Enter..." 
                ;;
            4) 
                read -p "Domain Baru: " nd
                echo "$nd" > /usr/local/etc/xray/domain
                openssl req -x509 -newkey rsa:2048 -nodes -sha256 -keyout $XRAY_DIR/xray.key -out $XRAY_DIR/xray.crt -days 3650 -subj "/CN=$nd" >/dev/null 2>&1
                systemctl restart xray
                echo "Domain Updated!"
                sleep 1 
                ;;
            ...
            6) 
                header_sub
                echo -e "${YELLOW}Running YABS (This will take a while)...${NC}"
            ...
            8) 
                clear
                echo -e "${YELLOW}AUTO REBOOT SETTING${NC}\n"
                read -p "Set Jam Auto Reboot (HH:MM, contoh: 05:00): " rb_time
                if [[ "$rb_time" =~ ^[0-9]{2}:[0-9]{2}$ ]]; then 
                    hr=$(echo $rb_time | cut -d: -f1)
                    mn=$(echo $rb_time | cut -d: -f2)
                    crontab -l 2>/dev/null | grep -v "/sbin/reboot" | crontab -
                    (crontab -l 2>/dev/null; echo "$mn $hr * * * /sbin/reboot") | crontab -
                    echo -e "${GREEN}Auto reboot set for $rb_time daily!${NC}"
                else 
