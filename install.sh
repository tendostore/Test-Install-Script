...
apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" curl socat jq openssl uuid-runtime net-tools vnstat wget gnupg1 bc iproute2 iptables iptables-persistent python3 neofetch cron stunnel4 bzip2 zlib1g-dev build-essential gcc make cmake >/dev/null 2>&1 & install_spin
...
# --- NEW: SSH, DROPBEAR 2019, WS & UDPGW ---
...
# Config Tanpa WARP/Routing, ditambah Fallback SSH WS (10015)
cat > $CONFIG_FILE <<EOF
{
  "log": { "loglevel": "warning" },
  "inbounds": [
    { "tag": "inbound-443", "port": 443, "protocol": "vless", "settings": { "clients": [ { "id": "$UUID_SYS", "flow": "xtls-rprx-vision", "level": 0, "email": "system" } ], "decryption": "none", "fallbacks": [ { "dest": 10015, "xver": 1 }, { "path": "/vmess", "dest": 10001, "xver": 1 }, { "path": "/vless", "dest": 10002, "xver": 1 }, { "path": "/trojan", "dest": 10003, "xver": 1 } ] }, "streamSettings": { "network": "tcp", "security": "tls", "tlsSettings": { "certificates": [ { "certificateFile": "/usr/local/etc/xray/xray.crt", "keyFile": "/usr/local/etc/xray/xray.key" } ] } } },
    { "tag": "inbound-80", "port": 80, "protocol": "vless", "settings": { "clients": [], "decryption": "none", "fallbacks": [ { "dest": 10015, "xver": 1 }, { "path": "/vmess", "dest": 10001, "xver": 1 }, { "path": "/vless", "dest": 10002, "xver": 1 }, { "path": "/trojan", "dest": 10003, "xver": 1 } ] }, "streamSettings": { "network": "tcp", "security": "none" } },
...
function header_main() {
    ...
    # Account Counters
    ACC_SSH=$(awk -F: '$3 >= 1000 && $1 != "nobody" {print $1}' /etc/passwd | wc -l)
...
function ssh_menu() {
    while true; do header_sub
        echo -e "${CYAN}│${NC} [1] Create Account SSH & WS"
        ...
        case $opt in
            1) read -p " Username : " u; read -p " Password : " p; read -p " Expired (days): " ex; [[ -z "$ex" ]] && ex=30; exp=$(date -d "$ex days" +"%Y-%m-%d"); useradd -e $(date -d "$ex days" +"%Y-%m-%d") -s /bin/false -M $u; echo -e "$p\n$p" | passwd $u >/dev/null 2>&1
               ...
               echo -e "Port Dropbear  : 90"
               echo -e "Port UDPGW     : 7100-7600"
               echo -e "Payload WS     : GET / HTTP/1.1[crlf]Host: $DMN[crlf]Upgrade: websocket[crlf][crlf]"
               echo -e "Expired On     : $exp"
               echo -e "————————————————————————————————————"
               read -n 1 -s -r -p "Tekan enter untuk kembali...";;
            2) read -p " Username to delete : " u; userdel -f $u 2>/dev/null; echo -e "${GREEN}Deleted!${NC}"; sleep 1;;
            3) read -p " Username to renew : " u; read -p " Add Days : " ex; chage -E $(date -d "$ex days" +"%Y-%m-%d") $u; echo -e "${GREEN}Renewed!${NC}"; sleep 1;;
            x) return;;
        esac; done
}
...
function features_menu() {
    while true; do header_sub
        ...
        case $opt in
            ...
            4) systemctl restart xray zivpn vnstat dropbear stunnel4 ws-proxy; echo -e "${GREEN}Services Restarted!${NC}"; sleep 2;;
...
while true; do header_main
    echo -e "${CYAN}│${NC} [1] SSH ACCOUNT          [5] ZIVPN UDP"
    echo -e "${CYAN}│${NC} [2] VMESS ACCOUNT        [6] CHANGE DOMAIN VPS"
    echo -e "${CYAN}│${NC} [3] VLESS ACCOUNT        [7] FEATURES"
    echo -e "${CYAN}│${NC} [4] TROJAN ACCOUNT       [8] CHECK SERVICES"
    ...
    read -p " Select Menu : " opt
    case $opt in
        1) ssh_menu ;; 2) vmess_menu ;; 3) vless_menu ;; 4) trojan_menu ;;
        5) zivpn_menu ;; 6) change_domain_menu ;; 7) features_menu ;;
        8) check_services ;;
        x) exit ;;
    esac; done
END_MENU
