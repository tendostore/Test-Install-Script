...
function maskStringTarget(str) {
    ...
    // Sensor semua karakter kecuali 2 karakter terakhir sesuai instruksi
...
app.post('/api/order', async (req, res) => {
    try {
        ...
        if (statusOrder === 'Sukses') {
            ...
            let timeStr = new Date().toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false });
            globalTrx.unshift({ time: timeStr, product: p.nama, user: (db[targetKey].username || targetKey), target: maskStringTarget(tujuan), price: hargaFix, method: 'Saldo Akun' });
...
async function executeVpnOrder(phone, protocol, productId, mode, vpnUsername, vpnPassword, expiredDays, refIdAsal = null, paymentMethod = 'Saldo Akun') {
    ...
    try {
        ...
        if(resApi.data && resApi.data.data) {
            ...
            let globalTrx = loadJSON(globalTrxFile);
            let timeStr = new Date().toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false });
            globalTrx.unshift({ time: timeStr, product: prodName, user: (db[targetKey].username || targetKey), target: maskStringTarget(apiData.username), price: hargaFix, method: (mode === 'trial' ? 'Gratis' : paymentMethod) });
            if(globalTrx.length > 100) globalTrx.pop();
            saveJSON(globalTrxFile, globalTrx);

            sendBroadcastSuccess(prodName, (db[targetKey].username || targetKey), apiData.username, hargaFix, mode === 'trial' ? 'Gratis' : paymentMethod);
...
app.post('/api/order-vpn-qris', async (req, res) => {
    try {
        ...
        res.json({success: true});
        sendTelegramAdmin(`🛒 <b>ORDER VPN QRIS PENDING</b>\n\n👤 Akun: ${db[targetKey].username || targetKey}\n📦 Produk: ${prodName}\n🎯 Username VPN: ${username}\n💰 Tagihan: Rp ${totalPay.toLocaleString('id-ID')}\n🔖 Ref: ${trxId}`);
...
async function prosesAutoOrderQRIS(phone, sku, tujuan, nama_produk, harga_asli, refIdAsal) {
    try {
        ...
        if (statusOrder === 'Sukses') {
            let globalTrx = loadJSON(globalTrxFile);
            let timeStr = new Date().toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false });
            globalTrx.unshift({ time: timeStr, product: nama_produk, user: (db[phone].username || phone), target: maskStringTarget(tujuan), price: hargaFix, method: 'QRIS' });
            if(globalTrx.length > 100) globalTrx.pop();
            saveJSON(globalTrxFile, globalTrx);

            sendBroadcastSuccess(nama_produk, (db[phone].username || phone), tujuan, hargaFix, 'QRIS');
        }
            ...
            try {
                ...
                if (resData.status === 'Sukses' || resData.status === 'Gagal') {
                    ...
                    if(resData.status === 'Sukses') {
                        ...
                        let globalTrx = loadJSON(globalTrxFile);
                        let timeStr = new Date().toLocaleTimeString('id-ID', { timeZone: 'Asia/Jakarta', hour12: false });
                        globalTrx.unshift({ time: timeStr, product: trx.nama, user: (db[phoneKey]?.username || phoneKey), target: maskStringTarget(trx.tujuan), price: parseInt(trx.harga), method: 'Sistem Otomatis' });
                        if(globalTrx.length > 100) globalTrx.pop();
                        saveJSON(globalTrxFile, globalTrx);

                        sendBroadcastSuccess(trx.nama, (db[phoneKey]?.username || phoneKey), trx.tujuan, parseInt(trx.harga), 'Sistem Otomatis');
