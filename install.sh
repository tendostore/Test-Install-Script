...
#           + Auto-Kill Expired & Limit Session SSH via Cron
#           + Added List Accounts (Count) on Dashboard
...
(
    ...
    apt-get install -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" curl socat jq openssl uuid-runtime net-tools vnstat wget gnupg1 bc iproute2 iptables iptables-persistent python3 neofetch cron zip unzip build-essential dirmngr zlib1g-dev bzip2
...
END
    ...
    # Python WS Proxy for SSH (Port 10010)
...
def handle_client(client_socket):
    try:
        ...
        # Balas handshake ke VPN App
        client_socket.sendall(b"HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n")
        
        target_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        target_socket.connect(('127.0.0.1', 90)) # Arahkan ke Dropbear
        
        # Lempar sisa payload jika ada
        idx = request.find(b'\r\n\r\n')
        if idx != -1:
            extra = request[idx+4:]
            if extra: target_socket.sendall(extra)
        else:
...
Type=simple
User=root
ExecStart=/usr/bin/python3 /usr/local/bin/ws-proxy.py 10010
Restart=always
[Install]
WantedBy=multi-user.target
END
...
print_ok "SSH & Multiplexer Backend"

# --- 6. XRAY CONFIG (MULTIPLEXING 443 & 80 to SSH) ---
...
# Script Expiry Auto-Kill (Xray, Zivpn & SSH)
...
# Script Limit IP (Xray & Multi-Login SSH)
...
if [[ -f "$SSH_FILE" ]]; then
    ...
    while IFS="|" read -r user pass exp limit status; do
        ...
        if [[ "$active_ssh" -gt 0 ]]; then
            PROTO_MSG+="👤 User: <code>$user</code> | 🌐 Login: $active_ssh Session aktif"$'\n\n'
            FOUND=1
        fi
    done < "$SSH_FILE"
    if [[ "$FOUND" -eq 1 ]]; then
        FULL_MSG+="<b>📊 LAPORKAN PENGGUNA AKTIF (SSH Dropbear)</b>"$'\n\n'"${PROTO_MSG}"
    fi
fi
...
# ---------------------------------------------
# FUNGSI OUTPUT DETAIL AKUN XRAY & SSH
# ---------------------------------------------
...
function show_account_ssh() {
    ...
    MSG+="Username       : ${user}\nPassword       : ${pass}\nCITY           : ${city}\nISP            : ${isp}\nDomain         : ${domain}\n"
    MSG+="Port TLS       : 443\nPort none TLS  : 80\nPort OpenSSH   : 22, 444\n"
    MSG+="Limit Login    : ${limit} Devices\nExpired On     : ${exp}\n"
    MSG+="————————————————————————————————————\n      Payload HTTP Custom & Injector\n————————————————————————————————————\n"
    MSG+="GET / HTTP/1.1[crlf]Host: domain_bug_anda.com[crlf][crlf]PATCH / HTTP/1.1[crlf]Host: ${domain}[crlf]Upgrade: websocket[crlf][crlf][split]HTTP/ 1[crlf][crlf]\n"
    MSG+="————————————————————————————————————\n"
    ...
    MSG_BOT+="Port TLS       : 443\nPort none TLS  : 80\nPort OpenSSH   : 22, 444\n"
    MSG_BOT+="Limit Login    : ${limit} Devices\nExpired On     : ${exp}\n"
    MSG_BOT+="<b>————————————————————————————————————</b>\n"
...
function header_main() {
    ...
    printf "${CYAN}│${NC} STATUS  : XRAY: %b ${CYAN}|${NC} SSH/DB: %b ${CYAN}|${NC} ZIVPN: %b\n" "$X_ST" "$D_ST" "$Z_ST"
    echo -e "${CYAN}└───────────────────────────────────────────────────────${NC}"
    ...
    echo -e "${CYAN}├───────────────────────────────────────────────────────${NC}"
    printf "${CYAN}│${NC} SSH DROPBEAR   : ${WHITE}%-4s${NC} ACCOUNT\n" "$ACC_SSH"
    printf "${CYAN}│${NC} VMESS X-RAY    : ${WHITE}%-4s${NC} ACCOUNT\n" "$ACC_VMESS"
    printf "${CYAN}│${NC} VLESS X-RAY    : ${WHITE}%-4s${NC} ACCOUNT\n" "$ACC_VLESS"
    printf "${CYAN}│${NC} TROJAN X-RAY   : ${WHITE}%-4s${NC} ACCOUNT\n" "$ACC_TROJAN"
    printf "${CYAN}│${NC} UDP ZIVPN      : ${WHITE}%-4s${NC} ACCOUNT\n" "$ACC_ZIVPN"
    echo -e "${CYAN}└───────────────────────────────────────────────────────${NC}"
}
...
# ---------------------------------------------
# MENU SSH & X-RAY MANAGER & FEATURES
# ---------------------------------------------

function ssh_menu() {
    while true; do header_sub
        echo -e "${CYAN}│${NC} [1] Create Account SSH Dropbear"
        echo -e "${CYAN}│${NC} [2] Delete Account SSH Dropbear"
        echo -e "${CYAN}│${NC} [3] Renew Account SSH Dropbear"
        echo -e "${CYAN}│${NC} [4] Check Config User"
        echo -e "${CYAN}│${NC} [5] Trial Account SSH Dropbear"
        ...
        case $opt in
            ...
            3) nl $D_SSH; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_SSH); u=$(echo "$line" | cut -d'|' -f1); p=$(echo "$line" | cut -d'|' -f2); exp_old=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5); read -p " Add Days: " add_days; exp_new=$(date -d "$exp_old + $add_days days" +"%Y-%m-%d"); usermod -e $exp_new "$u" &> /dev/null; sed -i "${n}s/.*/$u|$p|$exp_new|$limit|$stat/" $D_SSH; echo -e "${GREEN}Account $u Renewed until $exp_new!${NC}"; sleep 2;;
            ...
            6) nl $D_SSH; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_SSH); u=$(echo "$line" | cut -d'|' -f1); p=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5);
               if [[ "$stat" == "ACTIVE" ]]; then
                   usermod -L "$u" &> /dev/null; pkill -u "$u" dropbear; pkill -u "$u" sshd; sed -i "${n}s/.*/$u|$p|$exp|$limit|LOCKED/" $D_SSH; echo -e "${GREEN}Account $u Locked Successfully!${NC}"; sleep 2
               else echo -e "${RED}Account is already locked!${NC}"; sleep 2; fi;;
            7) nl $D_SSH; read -p "No: " n; [[ -z "$n" ]] && continue; line=$(sed -n "${n}p" $D_SSH); u=$(echo "$line" | cut -d'|' -f1); p=$(echo "$line" | cut -d'|' -f2); exp=$(echo "$line" | cut -d'|' -f3); limit=$(echo "$line" | cut -d'|' -f4); stat=$(echo "$line" | cut -d'|' -f5);
               if [[ "$stat" != "ACTIVE" ]]; then
                   usermod -U "$u" &> /dev/null; sed -i "${n}s/.*/$u|$p|$exp|$limit|ACTIVE/" $D_SSH; echo -e "${GREEN}Account $u Unlocked Successfully!${NC}"; sleep 2
               else echo -e "${YELLOW}Account is already Active!${NC}"; sleep 2; fi;;
            x) return;;
        esac; done
}
    ...
    while true; do header_sub
        ...
        case $opt in
            ...
            4) header_sub
               ...
               sleep 2;;
            5) systemctl restart xray dropbear zivpn ws-proxy vnstat; echo -e "${GREEN}Services Restarted!${NC}"; sleep 2;;
            ...
            10) 
               ...
               if [[ -n "$link_res" ]]; then
                   ...
                   if [[ -f "/root/restore.zip" ]]; then
                       ...
                       systemctl restart xray dropbear zivpn
...
function check_services() {
    ...
    if systemctl is-active --quiet xray; then X_ST="${GREEN}ON${NC}"; else X_ST="${RED}OFF${NC}"; fi
    if systemctl is-active --quiet dropbear; then D_ST="${GREEN}ON${NC}"; else D_ST="${RED}OFF${NC}"; fi
    if systemctl is-active --quiet ws-proxy; then P_ST="${GREEN}ON${NC}"; else P_ST="${RED}OFF${NC}"; fi
    if systemctl is-active --quiet zivpn; then Z_ST="${GREEN}ON${NC}"; else Z_ST="${RED}OFF${NC}"; fi
    ...
    printf "${CYAN}│${NC} Python WS Proxy : %b${NC}\n" "$P_ST"
...
while true; do header_main
    echo -e "${CYAN}│${NC} [1] SSH MANAGER          [4] CHECK SERVICES"
    echo -e "${CYAN}│${NC} [2] X-RAY MANAGER        [5] BOT TELEGRAM SETUP"
    echo -e "${CYAN}│${NC} [3] ZIVPN UDP            [6] FEATURES"
    echo -e "${CYAN}│${NC}                          [x] EXIT"
    echo -e "${CYAN}────────────────────────────────────────────────────────${NC}"
    echo -e "${CYAN}┌───────────────────────────────────────────────────────${NC}"
    echo -e "${CYAN}│${NC}  Version   :  v24.07.78                  ${NC}"
